...
Confirm that the parent resolved every template input before review. If an unresolved placeholder remains, or if the review artifact or a required reference is unavailable, return `BLOCKED` with the missing input instead of guessing.
